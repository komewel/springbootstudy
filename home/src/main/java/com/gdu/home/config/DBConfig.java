...
@MapperScan(basePackages = {"com.gdu.home.mapper"}) 			// @Mapper가 존재하는 클래스 패키지를 작성한다(이름도 BoardMapper)
@PropertySource(value = {"classpath:application.yaml"}) // 프로퍼티 파일에 적힌 속성을 읽어오자!, 리소스 파일아래 따로 폴더에 안넣어놨으니 이렇게 간단하게 적힌다.
@EnableTransactionManagement // transaction 처리를 허용하겠다. 이게 있어야 한다.
@Configuration
public class DBConfig {
	
	private final Environment env; // 스프링에서 따로 관리하는 클래스이다 
